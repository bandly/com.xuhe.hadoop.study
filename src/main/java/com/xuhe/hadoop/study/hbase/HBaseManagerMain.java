package com.xuhe.hadoop.study.hbase;
...
public class HBaseManagerMain {
    ...
    public static void main(String[] args) {
        ...
        conf.set("hbase.master", "10.3.50.1:60000");
        //conf.set("hbase.zookeeper.property.clientport", "2181");
        conf.set("hbase.zookeeper.quorum", "10.3.51.1,10.3.51.2,10.3.51.3");
        ...
        try {
            ...
            /* TableName tableName = TableName.valueOf("xuhe_1");
            HTableDescriptor tableDesc = new HTableDescriptor(tableName);
            HColumnDescriptor columnDesc = new HColumnDescriptor("cf_xuhe1");
            tableDesc.addFamily(columnDesc);
            
            admin.createTable(tableDesc);*/

            TableName tableName = TableName.valueOf("DM_BRAND_DC_STORAGE_SALE_DM_V2");
            ...
            HColumnDescriptor[] cf_arr = htd.getColumnFamilies();
            for (HColumnDescriptor hd : cf_arr) {
                System.out.println(hd.getNameAsString());
                String aa = hd.getValue("BRAND_ID");
                System.out.println(aa);
            }

            Collection<HColumnDescriptor> list = htd.getFamilies();
            System.out.println(list.size());
            for (HColumnDescriptor hd : list) {
                String name = new String(hd.getName(), "UTF-8");
                System.out.println(name);
                System.out.println(hd.getBlocksize());

                Map<ImmutableBytesWritable, ImmutableBytesWritable> map = hd.getValues();
                for (Map.Entry<ImmutableBytesWritable, ImmutableBytesWritable> entry : map
                    .entrySet()) {
                    System.out
                        .println(new String(entry.getKey().get()) + "      " + entry.getValue());
                }
            }
    ...
    private void putDatas(Connection connection) throws IOException {
        ...
        String[][] values = { { "Beijing Baidu Technology Co.", "220.181.57.217", "北京",
                                "1999年10月11日", "2015年10月11日" },
                              { "Hangzhou Alibaba Advertising Co.", "205.204.101.42", "杭州",
                                "1999年04月15日", "2022年05月23日" } };
        TableName tableName = TableName.valueOf(TABLE_NAME);
